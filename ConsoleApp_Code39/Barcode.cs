...
namespace ConsoleApp_Code39
{
    abstract internal class Barcode : ICodeBarcode
    {
        ...
        { 
            get => _text;
            set => _text = isValidText(value) ? value : "";//Console.WriteLine("Text is not supported to make barcode");
            // check possibility to code text in barcode. Need to implement Event of Exception here
        }
        
        abstract protected bool isValidText(string textToCheck); // realization may be different for different code systems
        
        protected Bitmap _image;
        virtual public Bitmap Image 
        { 
            get => _image;
            set => _image = isValidImage(value) ? value : null; //Console.WriteLine("File is not supported to decode");
        ...
        abstract protected bool isValidImage(Bitmap textToCheck); // realization may be different for different code systems
